...
#small_trains_date <- small_trains_date %>%
...
#small_trains_date %>%
 # group_by(arrival_station, month) %>%
  #summarize(monthly_total_trips =  sum(total_num_trips)) %>%
  #mutate(cum_total_trips = cumsum(monthly_total_trips)) %>%
  #ggplot() +
  #geom_line(aes(x = date, y = cum_total_trips, color = arrival_station)) +
...
garden_harvest %>% 
  ...
  mutate(cum_harvest_lb = daily_harvest_lb) %>%
  ungroup() %>% 
  complete(variety, 
           date, 
           fill = list(daily_harvest_lb = 0)) %>%
  mutate(cum_harvest_lb = fct_reorder(cum_harvest_lb, vegetable, +cum_harvest_lb))
